...
function juntarFeatureCollections(collections) {
  return {
    ...
    features: collections.flatMap((fc) => Array.isArray(fc && fc.features) ? fc.features : []),
  };
}
...
function normalizarResultadoShpjs(resultado) {
  ...
  if (Array.isArray(resultado)) {
    return juntarFeatureCollections(resultado.map((r) => (r && r.geojson) || r));
  }

  if (resultado.geojson && resultado.geojson.type === "FeatureCollection") {
...
async function lerZip(file) {
  // shpjs é ESM; por isso precisa ser importado dinamicamente mesmo em servidor .cjs.
  const modulo = await import("shpjs");
  const shp = modulo.default || modulo;
...
async function carregarPasta(tipo) {
  ...
  console.log(`Carregando ${tipo.toUpperCase()} em: ${dir}`);
  console.log(`Arquivos candidatos: ${preferidos.length}`);
  ...
  for (const file of preferidos) {
    try {
      ...
      if (fc && fc.features && fc.features.length) {
        ...
        console.log(`OK: ${file} (${fc.features.length} feições)`);
      }
    } catch (error) {
      const item = { file, error: error.message };
      erros.push(item);
      console.warn(`ERRO: ${file} - ${error.message}`);
    }
  }

  const geojson = juntarFeatureCollections(collections);
  ...
  return {
    ...
    arquivosLidos: lidos,
    erros,
    totalFeicoes: geojson.features.length,
    geojson,
  };
}
...
app.get("/api/base/:tipo", async (req, res) => {
  ...
  if (!PATHS[tipo]) {
    return res.status(404).json({ error: "Tipo de base inválido" });
  }
...
app.listen(PORT, () => {
  ...
  console.log("Pastas configuradas:");
  console.log(PATHS);
});
